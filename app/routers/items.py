...
def get_item(item_id: int, db: Session = Depends(get_db), current_user:UserDB=Depends(get_current_user)):
    return get_item_by_id_service(db, item_id)


@router.put("/items/{item_id}",response_model=ItemResponse) # Working
def update_item(item_id: int, item: Item, db: Session = Depends(get_db), current_user:UserDB=Depends(get_current_user)):
...
def get_item_by_name(item_name:str, db: Session = Depends(get_db), current_user:UserDB=Depends(get_current_user)):
...
def get_items_paginated(skip: int = 0, limit: int = 10, 
...
def filter_items(min_price: float = 0,max_price: float = 1000000,
                 db: Session = Depends(get_db),
                 current_user:UserDB=Depends(get_current_user)):
...
def sort_items(order_by: str = "price", db: Session = Depends(get_db), current_user:UserDB=Depends(get_current_user)): # price is specify as default parameter
...
def search_items(char: str, db: Session = Depends(get_db), current_user:UserDB=Depends(get_current_user)):
